...
do
    ...
    "
    export OUTPUT_DIR=$(python -m dapr.exps.keyphrases.args.colbertv2 $CLI_ARGS)
    mkdir -p $OUTPUT_DIR
    export LOG_PATH="$OUTPUT_DIR/logging.log"
    echo "Logging file path: $LOG_PATH"
    torchrun --nproc_per_node=4 --master_port=29502 -m dapr.exps.keyphrases.colbertv2 $CLI_ARGS > $LOG_PATH
done