...
from dapr.exps.prepending_titles.args.dragon_plus import (
    DRAGONPlusPrependingTitlesArguments,
)
...
if __name__ == "__main__":
    ...
    initialize_ddp()
    args = parse_cli(DRAGONPlusPrependingTitlesArguments)
    if is_device_zero():
        args.dump_arguments()
    dataset = DAPRDataLoader(
        DAPRDataConfig(data_name_or_path=args.data_dir, titled=True)
    ...
    RetrievedPassageIDList.dump_trec_csv(
        retrieval_results=retrieved, fpath=franked, system="dragon_plus"
    )
