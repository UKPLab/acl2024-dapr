...
--split=dev
"
export OUTPUT_DIR=$(python -m dapr.exps.bm25_doc_passage_fusion.args.colbertv2 $CLI_ARGS)
mkdir -p $OUTPUT_DIR
export LOG_PATH="$OUTPUT_DIR/logging.log"
echo "Logging file path: $LOG_PATH"
setsid nohup python -m dapr.exps.bm25_doc_passage_fusion.colbertv2 $CLI_ARGS > $LOG_PATH &